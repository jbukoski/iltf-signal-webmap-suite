...
def index(request):
    ...
    if not request.user.username == 'ssmt_user':
        return HttpResponseRedirect(reverse('iltf_index'))

    #bndry = models.boundary.objects.all()
    documents = models.Document.objects.all()
    upload_files = next(os.walk(os.path.join(os.path.dirname(path), 'media/ssmt/uploaded')))[2]
    upload_list = []
    ...
    for up_file in upload_files:
        ...
        if up_file != ".DS_Store":
            ...
            up_file_path = os.path.join(os.path.dirname(path), 'media/ssmt/uploaded/', up_file)
            raw_doc_json = open(os.path.join(os.path.dirname(path), 'media/ssmt/uploaded/', up_file)).read()
    ...
    return render(request, 'ssmt/index.html', {
        'title': 'Sioux St. Marie Tribe',
        #'bndry': bndry,
        'documents': documents
    })

def home(request):
    return HttpResponseRedirect(urlresolvers.reverse('admin:app_list', args=("ssmt/",)))
    ...
    try:
        if request.GET:
            ...
            if text_name == "boundary.geojson":
                ...
                download_file = open(os.path.join(os.path.dirname(path), 'media/ssmt/uploaded/boundary.geojson'), "rb")
                ...
                for document in all_docs:
                    ...
                    if document.docfile.name == text_name:
                        ...
                        download_file = open(os.path.join(os.path.dirname(path), "media/ssmt/uploaded/", short_name), "rb")
                        response = HttpResponse(download_file, content_type='application/force-download')
...
def sample_up_view(request):
    ...
    if request.method == 'GET':
        ...
        if request.FILES['docfile']:
            ...
            if form.is_valid():
                ...
                file_path = os.path.join('ssmt/uploaded/', this_file.name)
        ...
        return render(request, 'ssmt_index')
    ...
    return render(request, 'ssmt_index', context)
...
def boundary_view(request):
    boundary_json = serialize('geojson', models.boundary.objects.all(), geometry_field='geom', fields = ('objectid', 'joined', 'planid', 'name', 'type', 'statedarea', 'shape_leng', 'shape_area', 'category', 'address', 'city', 'wateracces', 'zip', 'geom', 'id'))
